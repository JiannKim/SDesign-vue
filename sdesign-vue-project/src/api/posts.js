...
const fetchMySounds = (token, next) => {
  const config = { headers: { token }, params: { next } };
  return posts.get("/get/my/soundList", config);
};

// 사운드 리스트 데이터 생성 api 요청
const createSounds = (formInfo, token) => {
...
// 데이터 삭제 api 요청
const removeItem = (removeInfo, token) => {
  const config = { headers: { token } };
  return posts.post("/remove/my/sound", removeInfo, config);
};

// 검색 api 요청
const searchSounds = (token, searchData, next) => {
  const config = { headers: { token }, params: { keyword: searchData, next } };
